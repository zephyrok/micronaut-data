...
abstract class AbstractMultitenancySpec extends Specification {
    ...
    def "test schema multitenancy"() {
        ...
        setup:
            Map<String, String> dataSourceProperties = getDataSourceProperties().collectEntries { [sourcePrefix() + '.default.' + it.key, it.value] }
    ...
    def "test datasource multitenancy"() {
        setup:
            Map<String, String> dataSourceProperties = getDataSourceProperties().collectEntries { [sourcePrefix() + '.foo.' + it.key, it.value] } +
                    getDataSourceProperties().collectEntries { [sourcePrefix() + '.bar.' + it.key, it.value] }
            EmbeddedServer embeddedServer = ApplicationContext.run(EmbeddedServer, properties + getExtraProperties() + dataSourceProperties + [
                    'spec.name'                                               : 'multitenancy',
